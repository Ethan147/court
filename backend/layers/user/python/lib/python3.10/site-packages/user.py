...
def create_or_update_user(
    ...
    inserted_user_account_id = _upsert_user_account(
        curs, cognito_user_id, first_name, last_name, email, gender_category, date_of_birth, gender_self_specify
    )
    _insert_terms_and_conditions(curs, inserted_user_account_id, terms_consent_version)
...
def insert_user_play_location(
    ...
    query = """
        ...
        (
            ...
            city, state, country, postal_code, location,
            created_at, is_active
        )
        values
            (%s, %s, %s, %s, %s, %s, %s, ST_SetSRID(ST_MakePoint(%s, %s), 4326), now(), true)
        on conflict (user_account_id, address_line_1, city, state, country, postal_code)
            do update set is_active = true
        returning id
    """
    values = (user_account_id, address_line_1, address_line_2, city, state, country, postal_code, longitude, latitude)
    curs.execute(query, values)
    inserted_user_play_location = curs.fetchone()[0]

    return inserted_user_play_location

def set_user_play_location_activation(
    curs: cursor, user_play_location_id: int, is_active: bool
) -> int:
    query = "update public.user_play_location set is_active = %s where id = %s"
    values = (is_active, user_play_location_id)
    curs.execute(query, values)
    altered_user_play_location = curs.fetchone()[0]

    return altered_user_play_location

def insert_user_mailing_address(
    curs: cursor,
    user_account_id: int,
    address_line_1: str,
    address_line_2: Optional[str],
    ...
    query = """
        ...
        (
            user_account_id, address_line_1, address_line_2,
            city, state, country, postal_code, created_at, is_active
        )
        values
        ...
        returning id
    """
    values = (user_account_id, address_line_1, address_line_2, city, state, country, postal_code)
    curs.execute(query, values)
    inserted_user_mailing_address = curs.fetchone()[0]

    return inserted_user_mailing_address

def set_user_mailing_address_activation(
    curs: cursor, user_mailing_address_id: int, is_active: bool
) -> int:
    query = "update public.user_play_location set is_active = %s where id = %s"
    values = (is_active, user_mailing_address_id)
    curs.execute(query, values)
    altered_user_mailing_address = curs.fetchone()[0]

    return altered_user_mailing_address
