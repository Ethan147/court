...
def get_prune_active_or_create_session(curs: cursor, user_id: int, device_identifier: str) -> str:
    active_session = get_prune_active_sessions(curs, user_id, device_identifier)

    if not active_session:
        active_session = [_create_user_session(curs, user_id, device_identifier)]

    return active_session[0]

def _create_user_session(curs: cursor, user_id: int, device_identifier: str, session_time: int = DEFAULT_SESSION_TIME) -> str:
    query = """
...
def get_prune_active_sessions(curs: cursor, user_id: int, device_identifier: str) -> List[str]:
    _prune_sessions(curs, user_id)
    query = """
        select session_uuid
...
def extend_session(curs: cursor, session_uuid: str, extension_reason: str, extend_hours: int = DEFAULT_SESSION_TIME) -> None:
...
def _prune_sessions(curs: cursor, user_id: int) -> None:
    """ prune expired sessions & make user there is max 1 active session per user & device identifier """
    _prune_expired_sessions(curs, user_id)
    _prune_extra_user_device_sessions(curs, user_id)

def _prune_extra_user_device_sessions(curs: cursor, user_id: int) -> None:
...
def _prune_expired_sessions(curs: cursor, user_id: int) -> None:
...
def remove_all_user_sessions(curs: cursor, user_id: int) -> None:
    query = """
        update public.user_session
            set is_active = false
            where user_account_id = %s
